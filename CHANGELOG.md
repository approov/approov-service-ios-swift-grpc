...
### Fixed
- `@authority` and `@target-uri` no longer double-prefix the scheme when the interceptor is constructed with a scheme-bearing hostname. The documented integration passes a bare host, but the exclusion matcher already tolerated both forms and the signing components did not, so `"https://api.example.com"` produced a `@target-uri` of `https://https://api.example.com/...` — a signature base no backend can reconstruct. Note the known limitation now recorded alongside it: a gRPC endpoint on a **non-default port** is not represented in `@target-uri`, because the port is given to `connect(host:port:)` and never reaches the interceptor, so message signing is supported on default-port endpoints only.
